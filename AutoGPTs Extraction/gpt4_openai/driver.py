...
class ChatGptDriver:
    ...
    def __init_browser(self) -> None:
        ...
        # Fechar todas as instâncias do Brave
        for proc in psutil.process_iter():
            if proc.name() == "brave.exe":
                proc.terminate()

        # Aguardar um pequeno intervalo para garantir que todas as instâncias sejam fechadas
        time.sleep(2)
        ...
        options.add_argument("--start-maximized")  # Adicione esta linha para iniciar maximizado
        if self.__proxy:
        ...
        try:
            ...
            btn_to_dismiss = WebDriverWait(self.driver, 3).until(
                EC.presence_of_element_located(
                    (
                        By.XPATH,
                        '//div[@role="dialog"]//button[@class="btn relative btn-primary"]',
                    )
                )
            )
            if btn_to_dismiss:
                ...
                if i != 2:
                    ...
                    btn = WebDriverWait(self.driver, 3).until(
                        EC.presence_of_element_located(
                            (
                                By.XPATH,
                                '//button[@class="btn relative btn-neutral ml-auto"]',
                            )
                        )
                    )
                else:
                    ...
                    btn = WebDriverWait(self.driver, 3).until(
                        EC.presence_of_element_located(
                            (
                                By.XPATH,
                                '//button[@class="btn relative btn-primary ml-auto"]',
                            )
                        )
                    )
                if btn:
        ...
        for _ in range(5):
            try:
                return self.driver.find_element(
                    By.XPATH, "//img[@alt='Generated by DALL·E']"
                ).get_attribute("src")
            except Exception as e:
                print(e)
                self.__sleep(0.5)
    ...
    def send_message(self, message: str, stream: bool = False) -> dict:
        ...
        try:
            ...
            regenerate_buttons = (
                By.XPATH,
                '/html/body/div[1]/div[1]/div[2]/main/div[1]/div[1]/div/div/div/div/div[3]/div/div/div[2]/div/div[2]/div/div/span[3]/button/span',
            )
